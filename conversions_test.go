...
func TestMinorUnitExponent(t *testing.T) {
	if got := payproviders.MinorUnitExponent(payproviders.CurrencyNGN); got != 2 {
		t.Fatalf("NGN exponent %d, want 2", got)
	}
	if got := payproviders.MinorUnitExponent(payproviders.CurrencyXOF); got != 0 {
		t.Fatalf("XOF exponent %d, want 0", got)
	}
	if got := payproviders.MinorUnitExponent(payproviders.CurrencyLYD); got != 3 {
		t.Fatalf("LYD exponent %d, want 3", got)
	}
}

func TestMinorToMajorUnit(t *testing.T) {
	if got := payproviders.MinorToMajorUnit(250000, payproviders.CurrencyNGN); got != 2500 {
		t.Fatalf("got %v, want 2500", got)
	}
}

func TestMajorUnitToMinor(t *testing.T) {
	if got := payproviders.MajorUnitToMinor(2500.50, payproviders.CurrencyNGN); got != 250050 {
		t.Fatalf("got %d, want 250050", got)
	}
}

func TestFormatMajorUnitAmount(t *testing.T) {
	if got := payproviders.FormatMajorUnitAmount(250000, payproviders.CurrencyNGN); got != "2500.00" {
...
func TestMinorFromGatewayAmount(t *testing.T) {
	...
	if got := payproviders.MinorFromGatewayAmount(250000, payproviders.CurrencyNGN, payproviders.NamePaystack); got != 250000 {
